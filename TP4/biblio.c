...
// lireFichier bonus fiche 4
bool lireFichier(struct Livre** t, FILE* f, int* nbreL, int* taille) {
   while(true) {
      if(*t == NULL){
         *t = (struct Livre*) malloc(MAX_NB_LIVRES*sizeof(struct Livre));
         if(t == NULL) return false;
         *taille = MAX_NB_LIVRES;
         *nbreL = 0;
      }
      else if(*taille < *nbreL + MAX_NB_LIVRES) {
         *t = realloc(t, 2*(*nbreL + MAX_NB_LIVRES)*sizeof(struct Livre));
         if(t == NULL) return false;
         *taille *= 2;
      }
      size_t nread = fread(*t+nbreL, sizeof(struct Livre), MAX_NB_LIVRES, f);
      *nbreL += nread;
      if(nread < MAX_NB_LIVRES) {
